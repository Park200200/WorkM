...
══════════════════════════════════════════════ */
/* ── 알림음 합성 (Web Audio API, 외부파일 없음) ── */
var _wsAudioCtx = null;
function _playNotifySound(type) {
  try {
    if (!_wsAudioCtx) _wsAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
    var ctx = _wsAudioCtx;
    /* AudioContext가 suspended 상태면 재개 */
    if (ctx.state === 'suspended') ctx.resume();

    function _beep(freq, startTime, dur, vol) {
    ...
    if (type === 'success') {
      /* 밝은 2음 상승 완료음 (솔 → 도) */
      _beep(784, t,        0.12, 0.10);  /* G5 */
      _beep(1047, t + 0.13, 0.18, 0.10); /* C6 */
    } else if (type === 'error') {
      /* 낮은 경고음 1음 */
      _beep(330, t, 0.22, 0.09); /* E4 */
    } else if (type === 'warning') {
      /* 중간 주의음 */
      _beep(523, t, 0.15, 0.07); /* C5 */
    }
  } catch(e) { /* AudioContext 미지원 환경 무시 */ }
}
