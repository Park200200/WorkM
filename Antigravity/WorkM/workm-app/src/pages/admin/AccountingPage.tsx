...
function AcctBudget({ year }: { year: number }) {
  ...
  const [budgetForm, setBudgetForm] = useState({ itemName: '', subName: '', accountCode: '', amount: '', memo: '' })
  ...
  const openBudgetModal = (editId?: number) => {
    if (editId) {
      ...
      if (b) {
        ...
        setBudgetForm({ itemName: b.itemName || '', subName: b.subName || '', accountCode: b.accountCode || '', amount: formatNumber(b.amount), memo: b.memo || '' })
      }
    } else {
      setBudgetEditId(null)
      setBudgetForm({ itemName: '', subName: '', accountCode: '', amount: '', memo: '' })
    }
  ...
  const saveBudgetItem = () => {
    ...
    if (budgetEditId) {
      const updated = all.map(b => {
        ...
        return { ...b, itemName: budgetForm.itemName.trim(), subName: budgetForm.subName.trim(), accountCode: budgetForm.accountCode, amount: amt, memo: budgetForm.memo }
      })
      ...
      all.push({
        ...
        subName: budgetForm.subName.trim(),
              ...
              <table className="w-full min-w-[700px]" style={{ tableLayout: 'fixed' }}>
                ...
                {(() => {
                  ...
                  return groupOrder.map(gk => {
                    ...
                    const gBal = gAmt - gSpent
                    const gPct = gAmt > 0 ? Math.round(gSpent / gAmt * 100) : 0
                    return (
                      <>
                        {/* 예산항목 소계 헤더 */}
                        <tr className="bg-[rgba(79,110,247,.05)] border-b border-[var(--border-default)]">
                          <td className="py-2.5 px-3.5 text-[12px] font-extrabold text-primary-600">
                            📋 {gk}
                            <span className="text-[10px] text-[var(--text-muted)] ml-2 font-bold">{items.length}건</span>
                          </td>
                          <td className="py-2.5 px-3.5" />
                          <td className="py-2.5 px-3.5 text-[12px] font-extrabold text-primary-600 text-right">{formatNumber(gAmt)}원</td>
                          <td className="py-2.5 px-3.5 text-[12px] font-extrabold text-danger text-right">{formatNumber(gSpent)}원</td>
                          <td className="py-2.5 px-3.5 text-[12px] font-extrabold text-right" style={{ color: gBal < 0 ? '#ef4444' : '#22c55e' }}>{formatNumber(gBal)}원</td>
                          <td className="py-2.5 px-3.5 text-[11px] font-bold text-[var(--text-muted)]">{gPct}%</td>
                          <td />
                        </tr>
                        {/* 세부 항목(계정과목별) */}
                        {items.map(b => {
                          ...
                          const displayName = b.subName || acctName || b.accountCode || '-'
                          return (
                            <tr key={b.id} className="border-b border-[var(--border-default)] last:border-0 hover:bg-[var(--bg-muted)] transition-colors">
                              <td className="py-2.5 px-3.5 text-[12px] font-bold text-[var(--text-primary)] pl-10">{displayName}</td>
          ...
          <div className="bg-[var(--bg-surface)] rounded-2xl shadow-2xl w-full max-w-[580px] mx-4 border border-[var(--border-default)] max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-default)]">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-extrabold text-[var(--text-primary)]">{editMode ? `${type === 'income' ? '입금' : '지출'} 수정` : `${type === 'income' ? '입금' : '지출'} 상세내역`}</span>
              </div>
            ...
            {editMode ? (() => {
              ...
              <div className="px-5 py-4 space-y-3">
                ...
                {/* 수정 히스토리 */}
                {((detailModal as any).editHistory || []).length > 0 && (
                  <div className="mt-3 pt-3 border-t border-[var(--border-default)]">
                    <div className="text-[10.5px] font-bold text-[var(--text-muted)] mb-2 flex items-center gap-1"><Clock size={11} /> 수정 이력</div>
                    <div className="space-y-2 max-h-[160px] overflow-y-auto">
                      {((detailModal as any).editHistory as { date: string; changes: string }[]).slice().reverse().map((h: { date: string; changes: string }, i: number) => (
                        <div key={i} className="flex gap-2 p-2 rounded-lg bg-[var(--bg-muted)] text-[11px]">
                          <span className="text-[var(--text-muted)] shrink-0 tabular-nums">{h.date.slice(0, 16).replace('T', ' ')}</span>
                          <span className="text-[var(--text-secondary)]">{h.changes}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="flex justify-between px-5 py-3.5 border-t border-[var(--border-default)]">
              {editMode ? (
                <>
                  <button onClick={() => setEditMode(false)} className="px-4 py-2 rounded-lg border border-[var(--border-default)] text-sm font-bold text-[var(--text-secondary)] hover:bg-[var(--bg-muted)] cursor-pointer">취소</button>
                  <button onClick={() => {
                    const cfs = getItem<CashFlow[]>('acct_cashflows', [])
                  ...
                  }} className={`flex items-center gap-1.5 px-5 py-2 rounded-lg text-white text-sm font-bold cursor-pointer bg-gradient-to-r ${typeGrads[type]}`}>
                    <Save size={14} /> 저장
                  </button>
                </>
              ) : (
                <>
                  ...
                  <button onClick={() => { setDetailModal(null); setEditMode(false) }} className="px-4 py-2 rounded-lg border border-[var(--border-default)] text-sm font-bold text-[var(--text-secondary)] hover:bg-[var(--bg-muted)] cursor-pointer">닫기</button>
                </>
              )}
            </div>
          </div>
