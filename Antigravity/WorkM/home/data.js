...
const WS = {
  ...
  users: JSON.parse(localStorage.getItem('ws_users')) || [
    { id:1001, name:'최대표', role:'대표', dept:'경영지원팀', pos:'CEO', avatar:'최대', color:'#4f6ef7',
      email:'ceo@workm.kr', phone:'010-1000-0001', birthday:'1975-03-15',
      hiredAt:'2015-01-01', resignedAt:null, address:'서울시 강남구 테헤란로 100',
      loginId:'admin', password:'1234', status:'근무', note:'대표이사', photo:'', approverType:'approver' },
    { id:1002, name:'박팀장', role:'팀장', dept:'개발팀', pos:'팀장', avatar:'박팀', color:'#9747ff',
      email:'park@workm.kr', phone:'010-1000-0002', birthday:'1980-07-20',
      hiredAt:'2018-03-01', resignedAt:null, address:'경기도 성남시 분당구 판교로 200',
      loginId:'park01', password:'1234', status:'근무', note:'개발팀 팀장', photo:'', approverType:'approver' },
    { id:1003, name:'하팀원', role:'사원', dept:'개발팀', pos:'팀원', avatar:'하팀', color:'#06b6d4',
      email:'ha@workm.kr', phone:'010-1000-0003', birthday:'1995-11-10',
      hiredAt:'2023-01-15', resignedAt:null, address:'서울시 마포구 월드컵로 300',
      loginId:'ha02', password:'1234', status:'근무', note:'개발팀 사원', photo:'', approverType:'requester' },
    { id:1004, name:'최경리', role:'대리', dept:'경영지원팀', pos:'팀원', avatar:'최경', color:'#22c55e',
      email:'choi@workm.kr', phone:'010-1000-0004', birthday:'1990-05-25',
      hiredAt:'2020-06-01', resignedAt:null, address:'서울시 서초구 서초대로 400',
      loginId:'choi03', password:'1234', status:'근무', note:'경영지원 대리', photo:'', approverType:'expense' },
    { id:1005, name:'강선임', role:'선임', dept:'기획팀', pos:'선임', avatar:'강선', color:'#f59e0b',
      email:'kang@workm.kr', phone:'010-1000-0005', birthday:'1988-09-08',
      hiredAt:'2019-04-01', resignedAt:null, address:'서울시 종로구 종로 500',
      loginId:'kang04', password:'1234', status:'근무', note:'기획팀 선임', photo:'', approverType:'requester' },
    { id:1006, name:'조영업', role:'과장', dept:'영업팀', pos:'팀원', avatar:'조영', color:'#ef4444',
      email:'jo@workm.kr', phone:'010-1000-0006', birthday:'1985-12-01',
      hiredAt:'2017-08-15', resignedAt:null, address:'서울시 강남구 학동로 600',
      loginId:'jo05', password:'1234', status:'근무', note:'영업팀 과장', photo:'', approverType:'requester' },
    { id:1007, name:'임기획', role:'대리', dept:'기획팀', pos:'팀원', avatar:'임기', color:'#8b5cf6',
      email:'lim@workm.kr', phone:'010-1000-0007', birthday:'1992-04-18',
      hiredAt:'2021-02-01', resignedAt:null, address:'경기도 고양시 일산서구 700',
      loginId:'lim06', password:'1234', status:'근무', note:'기획팀 대리', photo:'', approverType:'requester' },
    { id:1008, name:'오개발', role:'주임', dept:'개발팀', pos:'팀원', avatar:'오개', color:'#06b6d4',
      email:'oh@workm.kr', phone:'010-1000-0008', birthday:'1993-06-30',
      hiredAt:'2022-09-01', resignedAt:null, address:'서울시 송파구 올림픽로 800',
      loginId:'oh07', password:'1234', status:'근무', note:'개발팀 주임', photo:'', approverType:'requester' },
  ],
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ws_task_results에 icon/color 없는 항목이 있으면 강제 갱신 플래그
  const savedResults = JSON.parse(localStorage.getItem('ws_task_results') || '[]');
  const resultNeedsUpdate = savedResults.length > 0 && savedResults.some(r => !r.icon || !r.color);

  if (needsInit || resultNeedsUpdate) {
    fetch('./init-data.json?v=' + Date.now())
      .then(r => r.json())
      .then(data => {
        INIT_KEYS.forEach(k => {
          // ws_task_results: icon 없으면 무조건 덮어쓰기
          if (k === 'ws_task_results' && data[k]) {
            if (!localStorage.getItem(k) || resultNeedsUpdate) {
              localStorage.setItem(k, JSON.stringify(data[k]));
              WS.taskResults = data[k];
            }
          } else if (!localStorage.getItem(k) && data[k]) {
            localStorage.setItem(k, JSON.stringify(data[k]));
          }
        });
        // WS 객체 갱신
        ...
        console.log('[WorkM] init-data.json 로드 완료 (task_results 갱신:', resultNeedsUpdate, ')');
      })
