    ...
    <div className="animate-fadeIn flex flex-col h-full">
      ...
      <div className="flex-1 min-h-0 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-xl overflow-hidden flex flex-col">
        ...
        <div className="shrink-0 flex items-center gap-1.5 px-4 py-2 border-b border-[var(--border-default)] bg-[var(--bg-surface)]">
          {[
            { key: 'mine' as const, label: '나의업무', icon: <User size={14} />, count: scopeCounts.mine },
            { key: 'team' as const, label: '팀의업무', icon: <Users size={14} />, count: scopeCounts.team },
            { key: 'company' as const, label: '회사업무', icon: <Building2 size={14} />, count: scopeCounts.company },
          ].map(tab => (
            <button
              key={tab.key}
