        ...
        <ModalBody className="space-y-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div
                  ...
                  className="h-[44px] bg-[var(--bg-muted)] border-2 border-dashed border-[var(--border-default)] flex items-center justify-center gap-2 cursor-pointer hover:border-primary-400 transition-colors overflow-hidden"
                  style={{ borderRadius: 'var(--radius-sm)', ...(form.photo ? { backgroundImage: `url(${form.photo})`, backgroundSize: 'cover', backgroundPosition: 'center' } : {}) }}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <CustomSelect
                  value={form.rank || form.role || ''}
                  onChange={(v) => setForm({ ...form, rank: v, role: v })}
                  options={[
                    { value: '', label: '선택' },
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <CustomSelect
                  value={form.status || '근무'}
                  onChange={(v) => setForm({ ...form, status: v })}
                  options={[
                    { value: '근무', label: '근무' },
                    { value: '근무(휴가)', label: '근무(휴가)' },
                    { value: '근무(출장)', label: '근무(출장)' },
                    { value: '근무(조퇴)', label: '근무(조퇴)' },
                    { value: '근무(퇴근)', label: '근무(퇴근)' },
                    { value: '휴직', label: '휴직' },
                    { value: '퇴사', label: '퇴사' },
                  ]}
                  className="h-[44px]"
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] font-bold uppercase tracking-wider text-[var(--text-muted)]">생년월일</label>
                <DatePicker value={form.birthday || ''} onChange={v => setForm({ ...form, birthday: v })} placeholder="연도-월-일" className="h-[44px]" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] font-bold uppercase tracking-wider text-[var(--text-muted)]">입사일</label>
                <DatePicker value={form.hiredAt || ''} onChange={v => setForm({ ...form, hiredAt: v })} placeholder="연도-월-일" className="h-[44px]" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] font-bold uppercase tracking-wider text-[var(--text-muted)]">퇴사일</label>
                <DatePicker value={form.resignedAt || ''} onChange={v => setForm({ ...form, resignedAt: v })} placeholder="연도-월-일" className="h-[44px]" />
              </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full h-[44px] border border-[var(--border-default)] bg-[var(--bg-surface)] px-3 pr-10 text-sm leading-[44px] text-[var(--text-primary)] outline-none focus:border-primary-400"
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input type="color" value={form.color || '#4f6ef7'} onChange={e => setForm({ ...form, color: e.target.value })} className="w-full h-[44px] border border-[var(--border-default)] p-1 cursor-pointer" style={{ borderRadius: 'var(--radius-sm)' }} />
              </div>
              <div className="col-span-2 md:col-span-3 flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  className="w-full h-[44px] min-h-[44px] border border-[var(--border-default)] bg-[var(--bg-surface)] px-3 py-2 text-sm text-[var(--text-primary)] outline-none resize-y focus:border-primary-400"
                  style={{ borderRadius: 'var(--radius-sm)' }}
                />
              </div>
            </div>
          ...
          {/* ── 섹션: 지출승인 설정 ── */}
          <div>
            <div className="text-[12px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-2">지출승인 설정</div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[11px] font-bold uppercase tracking-wider text-[var(--text-muted)] block mb-1.5">지출승인 권한</label>
                <button
                  type="button"
                  onClick={() => setForm({ ...form, approverType: form.approverType === 'approver' ? '' : 'approver' })}
                  className="flex items-center gap-3 w-full h-[38px] px-3 border transition-all cursor-pointer"
                  style={{ borderRadius: 'var(--radius-sm)', borderColor: form.approverType === 'approver' ? 'var(--color-primary-500)' : 'var(--border-default)', background: form.approverType === 'approver' ? 'var(--color-primary-50)' : 'var(--bg-surface)' }}
                >
                  {/* 토글 스위치 */}
                  <div className={`relative w-9 h-5 rounded-full transition-colors ${form.approverType === 'approver' ? 'bg-primary-500' : 'bg-gray-300 dark:bg-gray-600'}`}>
                    <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${form.approverType === 'approver' ? 'translate-x-[18px]' : 'translate-x-0.5'}`} />
                  </div>
                  <span className={`text-[12px] font-bold ${form.approverType === 'approver' ? 'text-primary-600' : 'text-[var(--text-muted)]'}`}>
                    {form.approverType === 'approver' ? '승인 권한 있음' : '승인 권한 없음'}
                  </span>
                </button>
              </div>
              <div>
                ...
                <div
                  ...
                  className="h-[38px] bg-[var(--bg-muted)] border-2 border-dashed border-[var(--border-default)] flex items-center justify-center gap-2 cursor-pointer hover:border-primary-400 transition-colors overflow-hidden"
                  style={{ borderRadius: 'var(--radius-sm)', ...(form.sealImg ? { backgroundImage: `url(${form.sealImg})`, backgroundSize: 'contain', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' } : {}) }}
