...
import { Building2, Search, ChevronDown, Plus, Edit3, Trash2, Save, X, MoreHorizontal, Paperclip, ClipboardList, Coins, Settings, CreditCard, Monitor, Database, FileText, User } from 'lucide-react'
...
export default function AcctHQVendor() {
  ...
  return (
    <div className="animate-fadeIn">
      ...
      {/* 테이블 (데스크톱 뷰) */}
      <div className="hidden md:block rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] overflow-hidden">
                ...
                <tr><td colSpan={8} className="p-0">
                  <div className="px-8 py-4 bg-blue-50/50 dark:bg-blue-900/5 border-b border-[var(--border-default)]">
                    <div className="flex items-center gap-2 mb-3">
                      <span className="text-[13px] font-bold text-[var(--text-primary)] flex items-center gap-1.5"><ClipboardList size={14} className="text-primary-500" /> 청구 리스트</span>
                    </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-[var(--bg-muted)] border-t border-[var(--border-default)]">
          ...
          <span className="text-[12px] text-[var(--text-muted)] flex items-center gap-1.5"><Coins size={14} className="text-primary-500" /> 청구액: <b className="text-primary-500">{totalBill.toLocaleString()}원</b></span>
        </div>
      </div>
      ...
      <div className="block md:hidden space-y-3">
        {filtered.map((v, i) => (
          <div key={v.id} className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-4 space-y-3">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-1.5 cursor-pointer" onClick={() => setExpandedId(expandedId === v.id ? null : v.id)}>
                <span className="text-[11px] font-bold text-[var(--text-muted)]">No.{i + 1}</span>
                <ChevronDown size={14} className={cn('transition-transform text-[var(--text-muted)]', expandedId === v.id && 'rotate-180')} />
                <h4 className="font-bold text-[14px] text-[var(--text-primary)]">{v.company}</h4>
              </div>
              <div className="relative" onClick={e => e.stopPropagation()}>
                <button onClick={() => setMenuId(menuId === v.id ? null : v.id)} className="w-7 h-7 rounded-lg hover:bg-[var(--bg-muted)] flex items-center justify-center text-[var(--text-muted)] cursor-pointer"><MoreHorizontal size={14} /></button>
                {menuId === v.id && (
                  <div className="absolute right-0 top-8 z-50 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-xl shadow-lg py-1 min-w-[100px] animate-scaleIn">
                    <button onClick={() => { setMenuId(null); openEdit(v) }} className="w-full px-4 py-2 text-left text-[12px] text-[var(--text-primary)] hover:bg-[var(--bg-muted)] cursor-pointer flex items-center gap-2"><Edit3 size={12} /> 수정</button>
                    <button onClick={() => { setMenuId(null); deleteVendor(v.id) }} className="w-full px-4 py-2 text-left text-[12px] text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer flex items-center gap-2"><Trash2 size={12} /> 삭제</button>
                  </div>
                )}
            ...
            <div className="grid grid-cols-2 gap-y-2 text-[12px] border-t border-[var(--border-default)] pt-2.5">
              <div>
                <span className="text-[var(--text-muted)] block text-[10px]">대표자</span>
                <span className="text-[var(--text-secondary)] font-medium">{v.ceo}</span>
              </div>
              <div>
                <span className="text-[var(--text-muted)] block text-[10px]">사업자번호</span>
                <span className="text-[var(--text-secondary)] font-mono">{v.bizNo}</span>
              </div>
              <div>
                <span className="text-[var(--text-muted)] block text-[10px]">대표전화</span>
                <span className="text-[var(--text-secondary)]">{v.bizPhone}</span>
              </div>
              <div>
                <span className="text-[var(--text-muted)] block text-[10px]">담당자</span>
                <span className="text-[var(--text-secondary)]">{v.mgrName}</span>
              </div>
            </div>

            <div className="border-t border-[var(--border-default)] pt-2.5 space-y-1">
              <span className="text-[var(--text-muted)] block text-[10px]">사용솔루션</span>
              <div className="flex flex-wrap gap-1">
                {v.solutions.filter(s => s.enabled).map(s => (
                  <span key={s.key} className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-primary-100 text-primary-600 dark:bg-primary-900/20 dark:text-primary-400">{s.label}</span>
                ))}
            ...
            <div className="flex items-center justify-between border-t border-[var(--border-default)] pt-2.5 bg-[var(--bg-muted)]/30 -mx-4 -mb-4 p-3 rounded-b-xl">
              <div>
                <span className="text-[var(--text-muted)] block text-[10px]">사용료 청구액</span>
                <span className="font-extrabold text-[13px] text-[var(--text-primary)]">{(v.totalBill || 0).toLocaleString()}원</span>
              </div>
              <div className="text-right">
                <span className="text-[var(--text-muted)] block text-[10px]">미수금</span>
                <span className="font-bold text-[13px] text-orange-500">{(v.unpaid || 0).toLocaleString()}원</span>
              </div>
            </div>

            {/* 모바일 아코디언 청구 리스트 */}
            {expandedId === v.id && v.billings.length > 0 && (
              <div className="mt-3 pt-3 border-t border-dashed border-[var(--border-default)] space-y-2">
                <span className="text-[11px] font-bold text-[var(--text-primary)] flex items-center gap-1"><ClipboardList size={12} /> 청구 내역</span>
                <div className="space-y-2">
                  {(v.billings as any[]).map((b: any, bi: number) => (
                    <div key={bi} className="rounded-lg border border-[var(--border-default)] bg-[var(--bg-muted)]/50 p-2.5 text-[11px] space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-[var(--text-primary)]">{b.period}</span>
                        <span className={cn('px-1.5 py-0.5 rounded-full text-[9px] font-bold', b.status==='과금중'?'bg-red-100 text-red-500 dark:bg-red-900/20':b.status==='청구'?'bg-blue-100 text-blue-500 dark:bg-blue-900/20':'bg-green-100 text-green-600 dark:bg-green-900/20')}>{b.status}</span>
                      </div>
                      <div className="grid grid-cols-2 gap-1 text-[10px] text-[var(--text-secondary)]">
                        <div>월관리비: {b.mgmt || '-'}원</div>
                        <div>DB사용료: {b.db || '-'}원</div>
                        <div>Data건수: {b.data || '-'}원</div>
                        <div>수수료: {b.fee || '-'}원</div>
                      </div>
                      <div className="text-right font-extrabold text-[var(--text-primary)] border-t border-[var(--border-default)] pt-1 mt-1">총 {b.total}원</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {/* 모바일 하단 요약 바 */}
        <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-3 flex justify-between items-center text-[12px]">
          <span className="text-[var(--text-muted)]">총 <b className="text-[var(--text-primary)]">{filtered.length}건</b></span>
          <span className="text-[var(--text-muted)] flex items-center gap-1"><Coins size={12} /> 총 청구액: <b className="text-primary-500">{totalBill.toLocaleString()}원</b></span>
        </div>
      </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-5">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_220px] gap-5">
                <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5 space-y-3">
                  <SectionHeader icon={<Building2 size={16} />} title="기본 정보" color="#4f6ef7" />
                ...
                <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-4 flex flex-col">
                  <SectionHeader icon={<FileText size={16} />} title="사업자등록증" color="#f59e0b" />
                  <div className="flex-1 flex flex-col items-center justify-center min-h-[150px]">
                    ...
                    ) : (<div className="flex flex-col items-center gap-2 py-4 text-[var(--text-muted)]"><FileText size={32} className="text-[var(--text-muted)]" /><span className="text-[11px]">등록된 사업자등록증이 없습니다</span></div>)}
                  </div>
              ...
              {/* 담당자 정보 */}
              <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5 space-y-3">
                <SectionHeader icon={<User size={16} />} title="담당자 정보" color="#22c55e" />
                <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] overflow-hidden">
                <div className="flex items-center justify-between px-5 py-3 border-b border-[var(--border-default)]">
                  <div className="flex items-center gap-2"><Settings size={14} className="text-primary-500" /><span className="text-[12px] font-extrabold text-[var(--text-primary)]">사용 솔루션</span></div>
              ...
              {editVendor.billings.length > 0 && (() => {
                ...
                <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] overflow-hidden">
                  <div className="flex items-center justify-between flex-wrap gap-2 px-5 py-3 border-b border-[var(--border-default)]">
                    <div className="flex items-center gap-2"><CreditCard size={14} className="text-primary-500" /><span className="text-[12px] font-extrabold text-[var(--text-primary)]">결제 정보</span></div>
                    <div className="flex items-center gap-3 text-[11px] text-[var(--text-muted)]">
                      <span>사용솔루션: <b className="text-primary-500">{enabledSols || '-'}</b></span>
                      <span>⏱ 과금일자: <b>{curBill.period || '-'}</b></span>
                      <span className="flex items-center gap-1"><Coins size={11} className="text-primary-500" /> 총금액: <b className="text-primary-500">{curBill.total || '0'}원</b></span>
                      {editVendor.totalBill > 0 && <span>📊 단가수정</span>}
                    </div>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4">
                    {[
                      { icon: <Monitor size={12} className="text-[var(--text-muted)]" />, label: '월관리비(서버)', value: curBill.mgmt || '0', sub: '기본금액' },
                      { icon: <Database size={12} className="text-[var(--text-muted)]" />, label: 'DB사용료(단가:100M당 1,000원)', value: curBill.db || '0', sub: '25,000MB' },
                      { icon: <span className="text-[10px] font-bold text-[var(--text-muted)]">#</span>, label: '자료단가(10건당 1원)', value: curBill.data || '0', sub: (curBill.data || '0') + '건' },
                      { icon: <span className="text-[10px] font-bold text-[var(--text-muted)]">%</span>, label: '수수료(7%)', value: curBill.fee || '0', sub: '기간매출:500,000원', hl: true },
                    ].map((c, ci) => (
                      <div key={ci} className={cn('rounded-xl p-3 border', c.hl ? 'bg-orange-50 dark:bg-orange-900/10 border-orange-200 dark:border-orange-800' : 'bg-[var(--bg-muted)] border-[var(--border-default)]')}>
                        <div className="text-[10px] font-semibold text-[var(--text-muted)] mb-1 flex items-center gap-1 truncate">{c.icon} {c.label}</div>
              ...
              {/* 비고 */}
              <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5 space-y-3">
                <SectionHeader icon={<FileText size={16} />} title="비고" color="#8b5cf6" />
...
function SectionHeader({ icon, title, color }: { icon: React.ReactNode; title: string; color: string }) {
  return (
    <div className="flex items-center gap-2 mb-3 mt-1">
      <span className="text-sm flex items-center justify-center" style={{ color }}>{icon}</span>
