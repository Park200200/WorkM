...
function buildGantt() {
  ...
  return '<div class="section-card full-width">'
    ...
    +    '<div class="section-title">'+(window.innerWidth<=767?'미완료 내업무 리스트':'마감일 기준 미완료 나의 업무 차트')+'</div>'
...
function renderDrExecList() {
  ...
  tbody.innerHTML = _drExecReports.map(function(r) {
    ...
    var isMob = window.matchMedia && window.matchMedia('(max-width: 767px)').matches; // kept for reference
    return '<tr>' +
